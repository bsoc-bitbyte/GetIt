<template >
<section v-if="this.$store.state.cart.length>0">
<div class="container mx-auto mt-20 hidden lg:block">
    <div class="flex shadow-md my-10">
      <div class="w-3/4 bg-white px-10 py-10">
        <div class="flex justify-between border-b pb-8">
          <h1 class="font-semibold text-2xl">Shopping Cart</h1>
          <h2 class="font-semibold text-2xl">{{this.$store.state.cart.length }} Products</h2>
        </div>
        ...
        <div class="flex items-center hover:bg-gray-100 -mx-8 px-6 py-5" v-for="item in $store.state.cart" :key="item.pid">
...
<section class="py-12 sm:py-16 lg:py-20  flex lg:hidden">
        ...
        <div class="px-4 py-6 sm:px-8 sm:py-10">
          ...
          <div class="mt-6 text-center">
            <button type="button" class="group inline-flex w-full items-center justify-center rounded-md bg-[#EA454C] px-6 py-4 text-lg font-semibold text-white transition-all duration-200 ease-in-out focus:shadow">
              <nuxt-link :to="this.$store.state.cart.length > 0 ? '/checkOut' : ''">Checkout</nuxt-link>
